...
VALID_CRITERIA_CLF = {"gini", "entropy"}
VALID_CRITERIA_REG = {"mse", "mae"}
...
def make_classifier_estimator(
    ...
    VALID_CRITERIA_CLF = {"gini", "entropy"}
    if criterion not in VALID_CRITERIA_CLF:
        raise ValueError(f"Invalid classification criterion: {criterion}")
    # RandomForestClassifier
    if name == "rf":
        if backend == "custom":
...
def make_regressor_estimator(
    ...
    VALID_CRITERIA_REG = {"mse", "mae"}
    if criterion not in VALID_CRITERIA_REG:
        raise ValueError(f"Invalid regression criterion: {criterion}")
