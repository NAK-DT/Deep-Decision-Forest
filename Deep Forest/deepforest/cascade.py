...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    ...
    def _make_layer(self, **layer_args):
        print(f"[DEBUG] is_classifier(self) = {is_classifier(self)}")
    ...
    def fit(self, X, y, sample_weight=None):
        print("[DEBUG] model class =", self.__class__)
        print("[DEBUG] _estimator_type =", getattr(self, "_estimator_type", "NOT FOUND"))
        X, y = check_X_y(
            X,
            y,
            multi_output=True
...
class CascadeForestClassifier(ClassifierMixin, BaseCascadeForest):
