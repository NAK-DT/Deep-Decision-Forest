...
for i in range(10):
    ...
    ret_layerS_exh, ret_dataS_exh, best_testS_exh, best_featS_exh, best_indS_exh, best_layerS_exh = (
        backpropagationExhaustive.find_best_improvement_single(
            Layers[-2:], Data_sets[-3], Data_sets[-2], L3_train_accuracy, subsets[-2], distribution,
            random_trees_included
        )
    )
    ...
    if (best_testP_exh > best_testS_exh) & (accuracy - best_testP_exh != accuracy):
        print("Propagate > Singular")
        Layers = deepcopy(ret_layerP_exh)
        Data_sets[1:] = deepcopy(ret_dataP_exh)
        subsets = deepcopy(best_subsP_exh)
        accuracy = best_testP_exh
        print("Exhaustive Propagate accuracy", accuracy)

    elif (best_testS_exh > best_testP_exh) & (accuracy - best_testS_exh != accuracy):
        print("Propagate < Singular")
        Layers[-2:] = deepcopy(ret_layerS_exh)
        Data_sets[-2:] = deepcopy(ret_dataS_exh)
        for j in range(len(best_featS_exh)):
            subsets[-2][best_featS_exh[j]] = best_indS_exh[j].copy()
        subsets[-1] = deepcopy(best_layerS_exh)
        accuracy = best_testS_exh
        print("Exh Single accuracy", accuracy)
    else:
        print("Original best")
